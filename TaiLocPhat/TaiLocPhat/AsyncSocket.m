...
{
//   // NSLog(@"sdfsdfsdfsfsdfsfsd");
    curAppdelegate=(AppDelegate*)[[UIApplication sharedApplication]delegate];
...
- (BOOL)createStreamsToHost:(NSString *)hostname onPort:(UInt16)port error:(NSError **)errPtr
{
	...
	// NSLog(@"connected to %@:%d", hostname, port);
    return YES;
}
...
- (void)maybeDequeueRead
{
	...
	theFlags &= ~kDequeueReadScheduled;
	//// NSLog(@"maybe dequeue read......%@,.......%@",theCurrentRead,theReadStream);
	// If we're not currently processing a read AND we have an available read stream
    //while(theCurrentRead != nil)
	if((theCurrentRead == nil) && (theReadStream != NULL))
	{
      //  // NSLog(@" the read queue count: %d",[theReadQueue count]);
		if([theReadQueue count] > 0)
		{
			// Dequeue the next object in the write queue
			theCurrentRead = [[theReadQueue objectAtIndex:0] retain];
        //    // NSLog(@"data read:%@",[theReadQueue objectAtIndex:0]);
...
{
//    // NSLog(@" ...............has bytes available.........the flags %d ,ksocket %d,lenght partial reader %d",theFlags,kSocketHasBytesAvailable,[partialReadBuffer length]);
   // return YES;
...
//    // NSLog(@"completeCurrentRead");
    ...
    const char* inputstream = (const char*)[result bytes];
    NSUInteger length = [result length];
    
    if (length > 0) {
        char testOnline = inputstream[0];
        // NSLog(@"testOnline: %d", testOnline);
        if (length > 1) {
            NSUInteger index = 0;
            switch (testOnline) {
                case (char) 0x02:
                case (char) 0x04:
                    if (testOnline == 0x02) {
                        // NSLog(@"Read Package, in CONTENT_DATA");
                    } else {
                        // NSLog(@"Read Package, in GROUP_DATA");
                    }
                    ...
                    for (index = 1; index < length - 1; index ++) {
                        if (inputstream[index] == (char) 0xc0 && inputstream[index + 1] == (char) 0x80) {
                            int begin = 0;
                            int curLength = 0;
                            if (isFirst) {
                                isFirst = FALSE;
                                ...
                                begin = startP;
                                curLength = index - begin;
                            } else {
                                begin = startP + 1;
                                curLength = index - begin;
                            }
                            
                            short appID = (short) (((inputstream[begin] << 4) & 0xf0) | (inputstream[begin + 1] & 0x0f));
                            char itemID = inputstream[begin + 2];
                            char groupID = inputstream[begin + 3];
                            char serviceID = inputstream[begin + 4];
                            short segment = (short) inputstream[begin + 5];
                            NSRange range = NSMakeRange(begin + 6, curLength - 6);
                            
                            NSData *checkData = [result subdataWithRange:range];
                            NSString *strTemp = [[NSString alloc] initWithData:checkData encoding:NSUTF8StringEncoding];
                            
                            startP = index + 2;

                            // add to read queue
                            if (curAppdelegate.queue_read != nil) {
                                DataPackage* dataPackage = [[DataPackage alloc] initWithType:testOnline appid:appID itemid:itemID groupid:groupID serviceid:serviceID segment:segment data:strTemp];
                                [curAppdelegate.queue_read addObject:dataPackage];
                                NSLog(@"added (serviceID: %d, appID: %d, segment: %d) into queue_read --> queue size: %d", serviceID, appID, segment, [curAppdelegate.queue_read count]);
                            }
                        }
                    }
                    break;
...
//	// NSLog(@"complete current Write");
//    [self readDataWithTimeout:1000000000000000 tag:0];
...
{
	NSAssert(theCurrentWrite, @"Trying to complete current write when there is no current write.");
//	// NSLog(@"end current write");
	[theWriteTimer invalidate];
	theWriteTimer = nil;
	
	[theCurrentWrite release];
	theCurrentWrite = nil;
    //[self readDataWithTimeout:10000000 tag:0];
}

- (void)doWriteTimeout:(NSTimer *)timer
{
	#pragma unused(timer)
//	// NSLog(@"do Write time out!........");
	NSTimeInterval timeoutExtension = 0.0;
