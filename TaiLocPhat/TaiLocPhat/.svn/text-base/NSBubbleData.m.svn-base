...
{
#if !__has_feature(objc_arc)
    return [[[NSBubbleData alloc] initWithText:text date:date type:type username:username] autorelease];
#else
    return [[NSBubbleData alloc] initWithText:text date:date type:type username:username];
#endif    
}
...
- (id)initWithText:(NSString *)text date:(NSDate *)date type:(NSBubbleType)type username:(NSString*)username
{
    ...
    NSInteger fonSize, textBoxWitdh;
    CGSize size;
//    if(UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad){
    if([[UIDevice currentDevice] userInterfaceIdiom] == UIUserInterfaceIdiomPad){
        ...
        fonSize = [UIFont systemFontSize];
        textBoxWitdh = 200;
    }
    
    UIFont *font = [UIFont systemFontOfSize:fonSize];
    size = [(text ? text : @"") sizeWithFont:font constrainedToSize:CGSizeMake(textBoxWitdh, 9999) lineBreakMode:NSLineBreakByWordWrapping];
    ...
    return [self initWithView:label date:date type:type insets:insets username:username];
}
...
#else
    return [[NSBubbleData alloc] initWithImage:image date:date type:type username:username];
#endif    
}
...
- (id)initWithImage:(UIImage *)image date:(NSDate *)date type:(NSBubbleType)type username:(NSString*)username
{
    ...
    return [self initWithView:imageView date:date type:type insets:insets username:username];       
}
...
#else
    return [[NSBubbleData alloc] initWithView:view date:date type:type insets:insets username:username];
#endif    
}
